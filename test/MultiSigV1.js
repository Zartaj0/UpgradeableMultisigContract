...
describe.only("MultiSig", function () {
  ...
  beforeEach(async function () {
    ...
    const MultiSig = await ethers.getContractFactory("MultiSigV1");
    multiSig = await MultiSig.deploy();
    await multiSig.initialize([owner.address, owner1.address, owner2.address], 2)
    tx = {
      to: multiSig.address,
  ...
  describe("Submitting proposals", () => {
    ...
    it("owner can submit a valid proposal to add owner", async () => {
      await expect(multiSig.submitProposal(1, ownerToAdd.address, 0,00)).to.be.fulfilled;
